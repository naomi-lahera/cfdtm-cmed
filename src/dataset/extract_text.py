...
def extract_text_from_xml(xml_path):
    ...
    # 1️⃣ Extraer texto completo del <body>
...
def process_path(input, output_xml, output_jsonlist):
    ...
    files = [{"year": year.name,
              ...
              for year in os.scandir(output_xml) if year.is_dir() 
              for label in os.scandir(year.path) if label.is_dir()
              for xml in os.scandir(label.path) if xml.is_file()]
...
def main():
    ...
    process_path(args.input, args.output_xml, args.output_jsonlists)
